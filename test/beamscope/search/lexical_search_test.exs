defmodule Beamscope.Search.LexicalSearchTest do
  ...
  describe "search/3" do
    setup do
      dir =
        Path.join(
          System.tmp_dir!(),
          "beamscope_lexical_search_test_#{System.unique_integer([:positive])}"
        )
    ...
    test "returns every match, unbounded — capping happens in Beamscope.Repo after ranking, not here",
         %{dir: dir} do
    ...
    test "returns [] rather than crashing for a nonexistent repo_path" do
      assert LexicalSearch.search("/tmp/beamscope_definitely_does_not_exist", "pop_messages") == []
    end
  end
end
