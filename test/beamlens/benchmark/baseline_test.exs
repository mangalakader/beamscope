defmodule Beamlens.Benchmark.BaselineTest do
  use ExUnit.Case, async: true

  alias Beamlens.Benchmark.Baseline
  ...
  test "measure/2 degrades gracefully for a nonexistent repo_path" do
    assert Baseline.measure("/tmp/beamlens_definitely_does_not_exist", ["helper"]) == %{
             bytes: 0,
             text: "",
             capped?: false
           }
  ...
  test "measure/2 caps total bytes read rather than accumulating unbounded content" do
    dir =
      Path.join(
        System.tmp_dir!(),
        "beamlens_baseline_cap_test_#{System.unique_integer([:positive])}"
      )
