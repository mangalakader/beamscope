defmodule Beamlens.Benchmark.RunnerTest do
  @moduledoc """
  ...
  test "run/1 produces real, honest rows for every auto-discovered task" do
    ...
    for row <- result.rows do
      ...
      assert is_integer(row.beamlens_tokens)
      assert row.baseline_tokens >= 0
      assert row.beamlens_tokens > 0
    end
  ...
  test "Report.write/2 writes a real, timestamped Markdown file" do
    dir =
      Path.join(
        System.tmp_dir!(),
        "beamlens_benchmark_report_test_#{System.unique_integer([:positive])}"
      )
    ...
    assert content =~ "# Beamlens benchmark report"
    assert content =~ @repo
