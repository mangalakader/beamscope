defmodule Beamlens.Search.StoreExternalTest do
  @moduledoc """
  ...
  @dets_path Path.join([@repo, ".beamlens", "search.dets"])

  setup_all do
    assert :ok = Store.get_or_build(@repo)
    on_exit(fn -> File.rm_rf!(Path.join(@repo, ".beamlens")) end)
    :ok
  end
  ...
  test "a chunk with invalid UTF-8 text doesn't crash the shared Embeddings/Store singletons" do
    dir =
      Path.join(
        System.tmp_dir!(),
        "beamlens_mixed_encoding_#{System.unique_integer([:positive])}"
      )
    ...
    embeddings_pid = Process.whereis(Beamlens.Embeddings)
    store_pid = Process.whereis(Store)

    assert :ok = Store.get_or_build(dir)

    assert Process.whereis(Beamlens.Embeddings) == embeddings_pid
