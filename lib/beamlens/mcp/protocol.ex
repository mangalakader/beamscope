defmodule Beamlens.MCP.Protocol do
  ...
  protocol library — this project only needs three stateless tools, so a
  full client/server SDK (session tracking, SSE, batching, capability
  ...
  alias Beamlens.MCP.Tools.{FindCallPath, GetCallees, GetCallers}

  @protocol_version "2025-03-26"
  @tools [GetCallers, GetCallees, FindCallPath]
  ...
  defp tool_result(data) do
    %{"content" => [%{"type" => "text", "text" => Jason.encode!(data)}], "structuredContent" => data, "isError" => false}
  end
  ...
  defp err(id, code, message), do: {:ok, %{"jsonrpc" => "2.0", "id" => id, "error" => %{"code" => code, "message" => message}}}
