defmodule Beamlens.Embeddings do
  @moduledoc """
  ...
  `Beamlens.Callgraph.Store`'s lazy-build-on-first-use pattern, so starting
  ...
  2. **Per-call graph retracing.** `Nx.Serving.run/2` is Nx's documented
     ...
     `Beamlens.Embeddings.ServingSupervisor` in `Beamlens.Application`),
  ...
  # A single bad input (e.g. non-UTF-8 text — see Beamlens.Search.Store's
  ...
  defp start_serving(name, model_info, tokenizer, batch_size, sequence_length) do
    ...
    {:ok, _pid} =
      DynamicSupervisor.start_child(
        Beamlens.Embeddings.ServingSupervisor,
        {Nx.Serving,
