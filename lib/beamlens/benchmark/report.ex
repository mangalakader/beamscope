defmodule Beamlens.Benchmark.Report do
  @moduledoc """
  Renders one or more `Beamlens.Benchmark.Runner.run/1` results as a
  ...
  def render(results, generated_at \\ DateTime.utc_now()) do
    """
    # Beamlens benchmark report

    Generated #{DateTime.to_iso8601(generated_at)} by `mix beamlens.benchmark`.
    Token counts are real BPE counts (a vendored cl100k_base-equivalent
    tokenizer via the `tokenizers` Hex package — see
    `Beamlens.Benchmark.Tokenizer`), not an estimate. Baseline is a real
  ...
  defp render_table(rows) do
    header = "| Task | Description | Baseline tokens | Beamlens tokens | Reduction | Quality |"
    separator = "|---|---|---:|---:|---:|---|"
  ...
  defp render_row(row) do
    ...
    "| #{row.task} | #{row.description} | #{row.baseline_tokens} | #{row.beamlens_tokens} | " <>
