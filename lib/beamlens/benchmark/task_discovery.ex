defmodule Beamlens.Benchmark.TaskDiscovery do
  @moduledoc """
  Auto-picks representative call-graph/search tasks from a repo's built
  call graph, so `mix beamlens.benchmark` doesn't need hardcoded
  ...
  defp pick_search_code(graph) do
    case pick_highest_in_degree(graph) do
      ...
      qualified_name ->
        ...
        %{
          # Backtick the name: forces beamlens's own exact-match extraction
          # to pick it up regardless of naming convention (see
          # Beamlens.Search.LexicalSearch's documented single-bare-word
          # gap), and `exact_name_term` is the known-good literal grep term
          # for the *baseline* side — deliberately not re-derived via
          # beamlens's own term extraction, which would make the baseline
          # circular (testing beamlens's heuristic against itself instead
  ...
  # unresolved dynamic dispatch (see Beamlens.Callgraph.Extractor) — "?"
