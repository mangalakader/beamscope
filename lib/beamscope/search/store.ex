defmodule Beamscope.Search.Store do
  @moduledoc """
  Per-repo semantic search index: chunks a repo (via
  `Beamscope.Chunking.Pipeline`), embeds each chunk (via
  `Beamscope.Embeddings`), and persists `{key, vector, metadata}` rows to a
  DETS table at `<repo_path>/.beamscope/search.dets` — a build artifact of
  the target repo, parallel to `_build/`, meant to be gitignored there.

  DETS is the durable store; like `Beamscope.Callgraph.Store`, the built
  ...
  (see `Beamscope.Embeddings`) aren't installed.
  """
  ...
  alias Beamscope.Chunking.Pipeline, as: ChunkingPipeline
  ...
  defp build_and_persist(repo_path, table, opts) do
    ...
    if invalid_chunks != [] do
      Logger.warning(
        "beamscope: skipping #{length(invalid_chunks)} chunk(s) with invalid UTF-8 text while indexing #{repo_path}"
      )
    end
  ...
  defp dets_path_for(repo_path), do: Path.join([repo_path, ".beamscope", "search.dets"])
  defp table_name_for(repo_path), do: :"beamscope_search_#{:erlang.phash2(repo_path)}"
end
