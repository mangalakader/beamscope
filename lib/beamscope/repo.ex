defmodule Beamscope.Repo do
  ...
  use rather than reaching into `Beamscope.Callgraph.Store` or
  `Beamscope.Search.Store` directly. Both stores already cache/persist per
  ...
  alias Beamscope.Search.LexicalSearch
  alias Beamscope.Search.Store, as: SearchStore
  ...
  see `Beamscope.Callgraph.Graph.callers_with_locations/2`.
  """
  ...
  terms in the query — see `Beamscope.Search.LexicalSearch`, works with no
  ML deps installed) and `semantic_matches` (top-K similar chunks via
  `Beamscope.Search.Store.search/3`). Returned as two separate lists rather
  ...
  `exact_matches` is ranked using the call graph beamscope already builds: a
  ...
  def reindex(repo_path, opts \\ []) do
    with :ok <- validate_repo_path(repo_path) do
      ...
      if Beamscope.Embeddings.available?(), do: SearchStore.reindex(repo_path, opts)
      :ok
    end
  end
